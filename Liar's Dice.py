...
#I was unable with the time constraints to get this to work
#Ideally, it would run after each challenge and give each player a new hand of dice.
'''def reroll():
    global num_players
    global player_list
    global player_rolls
...
def player_turn(): #Runs every time it is the human player's turn
    ...
    try:
        ...
        if choice == 'b':
            print("Enter your bet in the form, [number of dice] [number on dice]: ")
            bet_input = input()
            current_bet = current_bet.split(" ")
            amt_of_dice = current_bet[0]
            dice_num = current_bet[1]
        elif choice == 'c':
            challenger = player_list[0]
            challenge(challenger)
    time.sleep(1)
...
def challenge(challenger):
    ...
    #for i in range(0,len(player_list)):
     #   print(player_list[i], player_rolls[i])
    actual = 0
    for i in player_rolls:
        for j in i:
            if j==dice_num:
                #print("Found a "+str(dice_num) + ", " + str(j) + " in " + str(i)) 
                actual+=1
    ...
    if int(current_bet[0]) <= actual:
        ...
        player_rolls[loser].pop()
        loser = player_list[loser]
        print( loser + " loses one die.")
        time.sleep(1)
        loser_index = player_list.index(loser)
        current_bet = []
        if player_rolls[loser_index] == []: #Same as above
            player_rolls.pop(loser_index)
            player_list.remove(loser)
            num_players -= 1
        #reroll()
    time.sleep(1)
...
def computer_turn(player): #Runs for computer's turn
    ...
    if current_bet == []:
        ...
        if int(current_bet[0]) > 3: #Challenges the bet when someone bets anything higher than three
            challenger = player
            challenge(challenger)
        else:
...
1. Troubleshooting is the worst.
I spent about two hours writing the code for this program, and then about 9 troubleshooting.
It did teach me how to look for specific problems in the code and pinpoint their source. I also learned
how to quit  a program while it's running courtesy of Daniel, which allowed me to test problems in the code
without having to run the entire program.

2. Time module and global variables.
This is pretty self explanitory. I learned how to use the time module to make programs more aesthetically
pleasing and to increase user readability when the program is running. I also used global variables for
pretty much the first time on this project. They were kind of complicated to deal with, especially when
all their names became more similar, but they allowed my functions to work better together.

3. Functions.
Obviously I had already used functions prior to this project. However, I had not used them to this extent,
nor had I used so many functions in correspondence with each other. This forced me to do much more searching
between functions to make the program work. The functions did improve writing, as they allowed me to know
exactly where to go to find something in the code. The hardest part was definitely getting the functions
to run when they were supposed to and to use all the global variables effectively.'''
